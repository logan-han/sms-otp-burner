...
describe('App Component', () => {
  ...
  test('displays messages when available', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('+61412345678')).toHaveLength(2); // Number display + message table
    }, { timeout: 2000 });
  ...
  test('handles 404 response from virtual-numbers gracefully', async () => {
    ...
    // Should not show error for 404 on virtual-numbers
  ...
  test('handles expired virtual number', async () => {
    ...
    // Should automatically lease a new number since the existing one is expired
    await waitFor(() => {
  ...
  test('displays messages with recipient number in table', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('+61412345678')).toHaveLength(2); // One in number list, one in message table
      expect(screen.getByText('Your OTP is 123456')).toBeInTheDocument();
      expect(screen.getByText('+61987654321')).toBeInTheDocument();
    });

    // Check for "To Number" column header
  ...
  test('displays messages sorted by latest first', async () => {
    ...
    await waitFor(() => {
      // Check that the newer message appears first in the table
    ...
    // Additional checks outside waitFor
  ...
  test('displays separate date and time columns for messages', async () => {
    ...
    // Check that Date and Time columns exist
    expect(screen.getByText('Date')).toBeInTheDocument();
    expect(screen.getByText('Time')).toBeInTheDocument();
  });

  test('refresh button is disabled when no subscription ID', async () => {
    // Mock fetch to return empty virtual numbers (no active subscription)
    fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ virtualNumbers: [] })
    });
