...
function App() {
  ...
  const fetchMessages = useCallback(async (subscriptionData = null) => {
    ...
    try {
      ...
      if (response && response.ok) {
        const data = await response.json();
        // Sort messages by receivedAt timestamp, latest first
  ...
  const handleLeaseNumber = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.ok) {
        ...
        // No response (undefined) - treat as network error
  ...
  useEffect(() => {
    ...
    fetchCurrentNumber().then((result) => {
      if (result && result.hasNumbers && result.numbers.length > 0) {
        ...
        // Successfully fetched but no numbers - auto-lease
        handleLeaseNumber();
      } else if (result && result.error && result.serverError) {
        // Server error occurred - don't auto-lease, just stop loading
        setIsLoading(false);
      } else {
        // Network error or fallback - auto-lease
        handleLeaseNumber();
      }
    });
                ...
                {leasedNumbers.map((number, index) => (
                  <div key={number.virtualNumber || number.msisdn} className="number-item">
                    <div className={`number-display ${leasedNumbers.length === 1 ? 'single-number' : ''}`}>
                      {leasedNumbers.length > 1 && <span className="number-label">#{index + 1}:</span>}
